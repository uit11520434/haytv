<?php
    class post{
        ...
        public static function user_likes() {

            $post_id = isset( $_POST['postID'] ) ? $_POST['postID'] : exit;
            $customID  = isset( $_POST['customID'] ) ? intval($_POST['customID']) : exit;
            $user_id = isset( $_POST[ 'author_id' ] ) ? $_POST[ 'author_id' ] : get_current_user_id();
            $resources=_core::method( '_resources' , 'get' );

            self::$post_id = $post_id;

            $user_likes = get_user_meta( $user_id, 'like' )[0];

            if ( is_null( $user_likes) || empty( $user_likes) ) { ?>
            ...
            <?php } else {
                ...
                if ( 0 == $post_id ) {
                    $remain_likes = $user_likes;
                } else {
                    $remain_likes_position = array_search( $post_id, $user_likes );
                    $remain_likes = array_slice( $user_likes, $remain_likes_position );
                }
                
                
                $wp_query = new WP_Query( array( 'post__in' => $remain_likes, 'post_type' => $resources[$customID]['slug'], 'posts_per_page' => POSTS_PER_AUTHOR_PAGE ) );
                ...
                if ( $wp_query -> max_num_pages > 1 ) { 
                    $post_id = $remain_likes[POSTS_PER_AUTHOR_PAGE]; ?>
                    <div class="clearfix author-get-more">
                        <div>
                            <p class="button newblue">      
                                <a id="get-more-voted" href="javascript:void(0)" onclick="javascript:likes.my( <?php echo $user_id;?> , <?php echo $post_id; ?> , [] , '<?php echo $customID;?>' , true )"><?php echo __( 'Get more' , _DEV_ );?></a>
                            </p>
                        </div>
                    </div>
                <?php }
        ...
        public static function my_likes(){
            ...
            // if votes were copied to users meta
            if ( true == get_option( 'user_meta_votes_copied' ) ) {
                self::user_likes();
            }
	   ...
	   static function get_embeded_video($video_id,$video_type,$autoplay = 0,$width = 610,$height = 443){
	...
	static function play_video($width=610, $height=443){
		$result = '';	
	...
	static function get_attached_files($post_id){
	    ...
	    static function get_audio_files($post_id){
        	$attached_file = '';
  			$attached_file_meta = _core::method('_meta','get', $post_id , 'format' );
