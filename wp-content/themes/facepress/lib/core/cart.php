<?php 
	class _cart {
		...
		static function get_btn($post_id, $cart_action='add_item'){
		...
		static function redirect_shopping_cart(){
		...
		static function product_exists($pid){
			$pid=$pid;
			$max=count($_SESSION['cart']);
			$flag=-1;
		...
		static function get_available_qty($pid, $post_id){
			$available_qty = 0;
		...
		static function update_qty($pid, $post_id , $changed_qty, $action = 'add'){ /*$action can be add OR remove*/
			$tickets = meta::get_meta( $post_id , 'tickets' );
		...
		static function get_shopping_cart_totals(){
		...
		static function show_cart_total(){
			echo self::get_shopping_cart_totals();
			exit;
		}
		
		static function get_shopping_cart_details(){
		...
		static function get_cart_details_updated(){
		...
		static function expresscheckout(){
			require_once 'paypal/expresscheckout.php';
		}
		
		static function confirm_payment(){
			require_once 'paypal/order_confirm.php';
			exit();
		}
		
		static function send_notif_email($customer_name,$transaction_Id,$attachment_tickets){
		...
		static function get_transaction_details($items_purchased,$transaction_Id){
						...
						foreach($cart_items as $cart_item){
							...
							if(is_array($registration) && sizeof($registration) && $registration['enable'] == 'yes' && $registration['use'] == 'yes' && $registration['value'] != '' && $registration['quantity'] >0 ){
								...
								foreach($cart_item['ticket_id'] as $ticket_id){
									...
									$ticket_link = add_query_arg( "uid", $current_user->ID, esc_url( get_permalink(_core::method("_settings","get","settings", "payment", "paypal", "return_url")) ) );
									$ticket_link = add_query_arg( "tid", $ticket_id, esc_url( $ticket_link ) );
		...
		static function save_transaction(){
