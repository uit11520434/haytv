        ...
        public static function set( $post_id = 0 ){
            ...
            if( is_user_logged_in () ){
                ...
                if ( !in_array( $post_id, $user_likes ) ) {
                    array_push( $user_likes, $post_id );
                } else {
                    $post_id_index = array_search( $post_id, $user_likes );
                    array_splice( $user_likes, $post_id_index, 1 );
                }

                update_user_meta( $user_id, 'like', $user_likes);
                /**/
            ...
            if( $user && $user_ip ){
                ...
                if( empty( $date ) ){
                    if( ( count( $likes ) >= (int)self::minLikes( $post_id ) ) ){
                        _meta::set( $post_id , 'hot_date' , time() );
                    }
                }else{
                    if( ( count( $likes ) < (int)self::minLikes( $post_id ) ) ){
        ...
        public static function generate(){
            ...
            if( isset( $resources[ $customID ] ) ){
                ...
                foreach( $wp_query -> posts as $post ){
                    ...
                    _meta::set( $post -> ID , 'hot_date' , time() );
                }
